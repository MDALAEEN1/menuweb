...
import 'package:menuweb/MenuPage/cartpage/cart_service.dart';
...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final taxes = subtotal * 0.00; // 10% ضريبة
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: selected
                ? colors
                      .primary // اللون الأساسي في الثيم
                : colors.textSecondary.withOpacity(0.3), // لون هادئ غير مزعج
            width: selected ? 2 : 1,
          ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // المجموع الكلي
              _summaryRow(S.of(context).total, total, colors),
              const SizedBox(height: 20),
  ...
  Future<void> _placeOrder() async {
    ...
    if (items.isEmpty) {
      _showErrorSnackBar(S.of(context).cartEmptyError, colors);
      return;
    }

    // التحقق من الحقول المطلوبة
    if (nameController.text.isEmpty) {
      _showErrorSnackBar(S.of(context).nameRequiredError, colors);
      return;
    }

    if (fulfillmentMethod == "delivery" && addressController.text.isEmpty) {
      _showErrorSnackBar(S.of(context).addressRequiredError, colors);
      return;
    }

    if (phoneController.text.isEmpty) {
      _showErrorSnackBar(S.of(context).phoneRequiredError, colors);
      return;
    }
    ...
    try {
      ...
      if (uid == null) {
        _showErrorSnackBar(S.of(context).userNotFoundError, colors);
        return;
      }
      ...
      final orderData = {
        ...
        // status + type
        "status": "pending",
        "fulfillmentMethod": fulfillmentMethod, // pickup | delivery
        // CUSTOMER INFO (داخل خريطة واحدة)
        "customer": {
        ...
        // CASHIER (فارغ لأنه طلب Web)
        "cashier": {"name": "", "uid": ""},

        // ITEMS (موحدة تمامًا مع POS)
      ...
      _showSuccessSnackBar(S.of(context).orderSuccess(orderNumber), colors);
      ...
      _showErrorSnackBar(S.of(context).orderFailed(e), colors);
    } finally {
